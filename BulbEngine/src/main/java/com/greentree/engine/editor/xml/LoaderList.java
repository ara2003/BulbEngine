...
public class LoaderList implements Iterable<Loader> {
	...
	public <R> R load(final Class<R> clazz, final String xmlValue) {
		Collection<Exception> exception = new ArrayList<>();
		...
		throw new UnsupportedOperationException(String.format("%s as %s in %s %s", xmlValue, clazz.getName(), get(clazz), exception));

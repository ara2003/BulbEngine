	...
	public static void poll(final int width, final int height) {
		...
		while(true) {
			...
			while(Keyboard.next()) if(Keyboard.getEventKeyState()) {
				...
				Game.getEventSystem().event(new KeyEvent(KeyEvent.EventType.keyPressed, i));
			}else {
				i = Input.resolveEventKey(Keyboard.getEventKey(), Keyboard.getEventCharacter());
				Input.nextRepeat[i] = 0L;
				Game.getEventSystem().event(new KeyEvent(KeyEvent.EventType.keyReleased, i));
			}
			while(true) while(true) {
				while(Mouse.next()) if(Mouse.getEventButton() >= 0) {
					if(Mouse.getEventButtonState()) {
						...
						Game.getEventSystem().event(new MouseEvent(MouseEvent.EventType.mousePressed,
						...
						Game.getEventSystem().event(new MouseEvent(MouseEvent.EventType.mouseReleased,
								Mouse.getEventButton(), Input.pressedX, Input.pressedY));
					}
				}else
					if(Mouse.isGrabbed() && Input.displayActive && (Mouse.getEventDX() != 0 || Mouse.getEventDY() != 0))
						if(Input.anyMouseDown())
							Game.getEventSystem().event(new MouseEvent(MouseEvent.EventType.mouseDragged, 0, 0,
									Mouse.getEventDX(), -Mouse.getEventDY()));
				else Game.getEventSystem().event(
				...
				if(Input.displayActive && !Mouse.isGrabbed()) {
					if(Input.lastMouseX != Input.getMouseX() || Input.lastMouseY != Input.getMouseY()) {
						if(Input.anyMouseDown())
							Game.getEventSystem().event(new MouseEvent(MouseEvent.EventType.mouseDragged, 0, 0,
									Mouse.getEventDX(), -Mouse.getEventDY()));
						else Game.getEventSystem().event(new MouseEvent(MouseEvent.EventType.mouseMoved, 0, 0,
								Mouse.getEventDX(), -Mouse.getEventDY()));
						Input.lastMouseX = Input.getMouseX();
						Input.lastMouseY = Input.getMouseY();
					}
				...
				//				TODO
				//				if(Input.keyRepeat) for(i = 0; i < 1024; ++i) if(Input.pressed[i] && Input.nextRepeat[i] != 0L
				//						&& System.currentTimeMillis() > Input.nextRepeat[i]) {
				//							Input.nextRepeat[i] = System.currentTimeMillis() + Input.keyRepeatInterval;
				//							Input.consumed = false;
				//							for(final KeyListener listener : Input.keyListeners) {
				//								listener.keyPressed(i);
				//								if(Input.consumed) break;
				//							}
				//						}

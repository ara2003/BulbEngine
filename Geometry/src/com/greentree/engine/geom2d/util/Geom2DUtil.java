...
public final class Geom2DUtil {
	
	public static Point2D contact(final Line2D a, final Line2D b, final boolean border) {
	...
	public static Point2D contactLine(final Line2D line, final Line2D line_no_border) {
		final Point2D p = Geom2DUtil.contact(line, line_no_border, false);
	...
	public static Point2D rey(final Point2D point, final Vector2f vec, final float maxDistanse, final List<Shape2D> world) {
		...
		final Line2D scanner = new Line2D(point.getX(), point.getY(), point.getX() + (vec.x * maxDistanse),
	...
	public static Point2D reyCast(final Point2D point, final Vector2f vec, float maxDistanse, final List<Shape2D> world) {
		...
		while((t-- > 0) && (maxDistanse > 0)) {
			dis = Geom2DUtil.distanse(p, world) * 0.99f;
			maxDistanse -= dis;
			p.add(vec.mul(dis));
			if(dis < 1) return p;
		}
	...
	@Deprecated
	public static List<Line2D> toLine(final List<Point2D> points, boolean stripe) {
		final List<Line2D> res = new ArrayList<>();
		for(int i = 0; i < points.size()-1; i++) res.add(new Line2D(points.get(i), points.get(i + 1)));
		if(stripe)res.add(0, new Line2D(points.get(0), points.get(points.size() - 1)));
		return res;
	}

	public static boolean isIntersect(Shape2D a, Shape2D b) {
		...
		for(Point2D p : b.getPoints()) {
			if(a.isInside(p))return true;
		}
		return false;
	}

	public static float areaTriangle(Point2D a, Point2D b, Point2D c) {

...
public class GLShaderProgram extends ShaderProgram {
	...
	public VideoBuffer createVideoBuffer(final ByteBuffer data, final Type type, final Usage usage) {
		final VideoBuffer result = new GLVideoBuffer(type, usage, DataType.BYTE);
		result.setData(data);
	...
	public VideoBuffer createVideoBuffer(final DoubleBuffer data, final Type type, final Usage usage) {
		final VideoBuffer result = new GLVideoBuffer(type, usage, DataType.DOUBLE);
		result.setData(data);
		this.buffers.push(result);
		return result;
	}
	...
	public VideoBuffer createVideoBuffer(final IntBuffer data, final Type type, final Usage usage) {
		final VideoBuffer result = new GLVideoBuffer(type, usage, DataType.UNSIGNED_INT);
		result.setData(data);
		this.buffers.push(result);
		return result;
	}
	...
	public void passVertexAttribArray(final int attrNo, final VideoBuffer vbo, final boolean normalized, final int vertexSize, final int stride,
		...
		vbo.bind();
		GL20.glVertexAttribPointer(attrNo, vertexSize, Decoder.glType(vbo.getDataType()), normalized, byteStride, componentOffset);
		GLShaderProgram.validateOpenGL();
	...
	public static class Builder extends ShaderProgram.Builder {
		...
		public GLShaderProgram build() {
			if(!this.shaders.stream().anyMatch(s->GLSLShader.Type.VERTEX == s.getType())) throw new IllegalStateException("Vertex shader is manadatory");
			if(!this.shaders.stream().anyMatch(s->GLSLShader.Type.FRAGMENT == s.getType())) throw new IllegalStateException("Fragment shader is manadatory");
			if(this.shaders.stream().anyMatch(s->GLSLShader.Type.TESS_CONTROL == s.getType())
				&& !this.shaders.stream().anyMatch(s->GLSLShader.Type.TESS_EVALUATION == s.getType()))
				throw new IllegalStateException("Tesselation control shader exist, but no tesselation evaluation shader load");
			else if(!this.shaders.stream().anyMatch(s->GLSLShader.Type.TESS_CONTROL == s.getType())
				&& this.shaders.stream().anyMatch(s->GLSLShader.Type.TESS_EVALUATION == s.getType()))

...
public final class GLSLShader extends Shader {
	...
	private GLSLShader(final int id, final Type type) {
		this.id   = id;
		this.type = type;
	}
	...
	public static GLSLShader load(final InputStream source, final Type type) {
		return GLSLShader.load(GLSLShader.loadSource(source), type);
	}
	
	private static GLSLShader load(final String source, final Type type) {
		final int id = GL20.glCreateShader(Decoder.glType(type));
